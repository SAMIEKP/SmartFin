...
export const ProductDetailsView: React.FC<ProductDetailsViewProps> = ({
  ...
  if (!product) {
    return (
      <div className="p-12 text-center bg-white rounded-2xl border border-[#bcc9c6]/30">
        <h2 className="text-xl font-bold text-[#0b1c30]">No Product Selected</h2>
        <button
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'eligibility' | 'documents' | 'repayment'>('overview');
  const [calcAmount, setCalcAmount] = useState<number>(product.minAmount || 2000000);
  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      <nav className="flex items-center gap-2 text-xs font-semibold text-[#3d4947]">
        <button onClick={() => onNavigate('loan-products')} className="hover:text-[#00685f]">
          Loan Products
        </button>
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl border border-[#bcc9c6]/30 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-3 max-w-2xl">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2.5 py-0.5 rounded-full flex items-center gap-1">
              ★ {product.rating || 4.8} ({product.reviewsCount || 124} verified reviews)
            </span>
          ...
          <h1 className="text-3xl font-extrabold text-[#0b1c30]">{product.name}</h1>
          <p className="text-sm font-semibold text-[#3d4947]">Offered by {product.provider}</p>

          <div className="flex flex-wrap gap-2 pt-2">
            {product.tags.map((t, idx) => (
              <span key={idx} className="text-xs bg-[#eff4ff] text-[#00685f] px-2.5 py-1 rounded-md font-semibold">
                #{t}
              </span>
            ))}
          </div>
        ...
        <div className="bg-[#f4fffc] p-5 rounded-2xl border border-[#008378]/30 text-center space-y-3 w-full md:w-64 shrink-0">
          ...
          <div>
            <span className="text-[10px] text-gray-500 uppercase block">Interest Rate</span>
            <span className="text-2xl font-extrabold text-[#00685f]">{product.rateDisplay}</span>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-[#bcc9c6]/30 shadow-xs space-y-4">
            {activeTab === 'overview' && (
              <div className="space-y-4 text-xs text-[#3d4947] leading-relaxed">
                <h3 className="text-base font-extrabold text-[#0b1c30]">About {product.name}</h3>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 border-t border-gray-100">
                  ...
                  <div className="p-3 bg-[#eff4ff] rounded-xl">
                    <span className="text-[10px] text-gray-500 block">Max Limit</span>
                    <span className="font-bold text-[#0b1c30] text-sm">
                  ...
                  <div className="p-3 bg-[#eff4ff] rounded-xl">
                    ...
                    <span className="font-bold text-[#00685f] text-sm">{product.rateDisplay}</span>
                  </div>

                  <div className="p-3 bg-[#eff4ff] rounded-xl">
                    <span className="text-[10px] text-gray-500 block">Max Term</span>
                    <span className="font-bold text-[#0b1c30] text-sm">{product.termDisplay}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'eligibility' && (
              <div className="space-y-4">
                <h3 className="text-base font-extrabold text-[#0b1c30]">Eligibility Criteria</h3>
                <ul className="space-y-3">
                  {product.eligibility.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-xs text-[#0b1c30]">
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {(product.repaymentScheduleSample || [
                        { month: 1, principal: 150000, interest: 18000, balance: 4850000 },
                        { month: 2, principal: 150000, interest: 17500, balance: 4700000 },
                        { month: 3, principal: 150000, interest: 17000, balance: 4550000 },
                      ]).map((row) => (
                        <tr key={row.month} className="hover:bg-gray-50">
                          <td className="p-3 font-bold text-[#0b1c30]">Month {row.month}</td>
                          <td className="p-3 text-[#0b1c30]">MWK {row.principal.toLocaleString()}</td>
                          <td className="p-3 text-[#00685f]">MWK {row.interest.toLocaleString()}</td>
          ...
          <div className="bg-white p-6 rounded-2xl border border-[#bcc9c6]/30 shadow-xs space-y-4">
            <h3 className="text-base font-extrabold text-[#0b1c30]">Frequently Asked Questions</h3>
            <div className="space-y-3">
              {faqs.map((faq, idx) => (
                <div key={idx} className="border border-[#bcc9c6]/30 rounded-xl overflow-hidden">
                  <button
          ...
          <div className="bg-white p-6 rounded-2xl border border-[#bcc9c6]/30 shadow-xs space-y-5 sticky top-20">
            ...
            <div>
              <div className="flex justify-between text-xs font-bold text-[#0b1c30] mb-1">
                <span>Amount:</span>
                <span className="text-[#00685f]">MWK {calcAmount.toLocaleString()}</span>
              </div>
              <input
                type="range"
            ...
            <div className="p-3 bg-gray-50 rounded-xl flex items-center gap-3 text-xs text-[#3d4947]">
              ...
              <div>
                <span className="font-bold block text-[#0b1c30]">256-Bit SSL Encrypted</span>
                <span className="text-[10px]">Your personal data is transmitted securely to the lender.</span>
              </div>
            </div>
          </div>
        </div>
