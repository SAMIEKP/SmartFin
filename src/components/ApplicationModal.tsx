...
export const ApplicationModal: React.FC<ApplicationModalProps> = ({
  ...
  const [amount, setAmount] = useState<number>(defaultProd?.minAmount || 1500000);
  const [monthlyIncome, setMonthlyIncome] = useState<number>(450000);
  const [fullName, setFullName] = useState<string>('Kwesi Banda');
  const [nationalId, setNationalId] = useState<string>('MW-88209411-B');
  const [phone, setPhone] = useState<string>('+265 999 123 456');
  const [location, setLocation] = useState<string>('Lilongwe');
